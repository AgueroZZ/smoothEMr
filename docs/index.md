...
The older `csmooth_em` and `smooth_em` algorithms are kept only as
lower-level compatibility helpers and are no longer part of the main
package workflow.
...
[`fit_mpcurve()`](https://aguerozz.github.io/MPCurver/reference/fit_mpcurve.md).
The public wrapper is CAVI-first, so there is no longer an `algorithm`
switch here.
...
[`fit_mpcurve()`](https://aguerozz.github.io/MPCurver/reference/fit_mpcurve.md)
returns either a single `mpcurve` fit or a list of fits. Each fit keeps
the unified `params`, `gamma`, `elbo_trace`, `loglik_trace`, and the
underlying raw fit in `$fit`.
...
helper is still available for explicit two-fit initialization workflows.
