...
This vignette introduces the statistical model, the variational algorithm, and
then walks through a larger but still easy spiral example.

The legacy `csmooth_em` pipeline remains available only through lower-level
compatibility functions and is no longer part of the public wrapper workflow.
...
The `cavi()` routine performs coordinate-ascent variational inference on an
explicit ELBO. Write
\[
r_{ik} := q_i(c_i = k),
\qquad
...
This is the quantity recorded in `elbo_trace`. The updates used in `cavi()`
are obtained by maximizing this ELBO one block at a time.
...
This is exactly the linear-algebra step implemented in `cavi()`.
...
The built-in `mpcurve` plotting methods still work on top of the default `cavi`
backend.
