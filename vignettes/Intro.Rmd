---
title: "Introduction to MPCurver"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Introduction to MPCurver}
...
recovers a latent ordering of the $n$ samples by fitting a Gaussian mixture
model whose component means are constrained to vary smoothly along an ordered
axis.

The main entry point is `fit_mpcurve()`, which now exposes a CAVI-first public
interface. Legacy `smooth_em` and `csmooth_em` routines remain available as
lower-level functions for historical comparison, but are not part of the
...
1. Each sample $i$ has a latent position $t_i \in \{1,\dots,K\}$ along an
   ordered grid of $K$ components.
...
3. Observations follow a Gaussian likelihood:
   ...
   N(u_{jk},\; \sigma_j^2).
   $$
4. Each trajectory is regularised by a Gaussian Markov random field (GMRF)
   prior:
   $$
   u_j \sim N\!\bigl(0,\; (\lambda_j\, Q_K)^{-1}\bigr),
   $$
   where $Q_K$ is a random-walk precision matrix on the $K$ grid points and
   $\lambda_j > 0$ controls the smoothness of feature $j$.

Both $\sigma_j^2$ and $\lambda_j$ are feature-specific and are estimated
from the data.

## The algorithm

`MPCurver` uses coordinate-ascent variational inference (CAVI).
The posterior over latent positions and trajectories is approximated by a
mean-field family:
$$
q(T,U)
...
CAVI maximises the evidence lower bound (ELBO) one block at a time.
Each coordinate update has a closed-form solution:
...
update. This is what distinguishes the variational approach from a simpler
plug-in EM algorithm.
...
```{r plot-data, fig.cap="Simulated spiral coloured by true pseudotime."}
...
The main function is `fit_mpcurve()`. We choose `K = 60` grid points and
the Fiedler (spectral) initialisation:
...
```{r plot-elbo, fig.cap="ELBO trace showing monotone convergence."}
plot(fit, plot_type = "elbo")
```

### Ordering recovery

We can extract a pseudotime estimate from the posterior responsibilities:
$$
...
```{r plot-ordering, fig.cap="True pseudotime (left) vs inferred pseudotime (right)."}
...
The posterior mean trajectories $m_j$ show how each feature changes along the
ordering:

```{r plot-mu, fig.cap="Posterior mean trajectories for each feature across the K grid points."}
...
feature via the variational update. You can fix it at a constant value using
`do_mpcurve()`:

```{r fix-lambda, eval=FALSE}
...
- `"fiedler"` (spectral ordering) is a good default for moderate non-linearity.
- `"isomap"` can be better for strongly curved manifolds.

## Continuing a fit

Use `do_mpcurve()` to run additional iterations on an existing fit:
...
- `fit_mpcurve()` is the main entry point.
- The CAVI algorithm maintains full posterior uncertainty over feature
  trajectories, with a monotone ELBO guarantee.
- Use `plot()` to visualise scatterplots, ELBO traces, and trajectory
  profiles.
- Use `do_mpcurve()` to continue fitting, fix lambda, or restrict parameter
  bounds.
- For dual-ordering / feature partition problems, see `vignette("partition")`.
